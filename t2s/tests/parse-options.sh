...
function show_usage {
   echo "Options: (devcloud|local) (gemm|conv|capsule|pairhmm|gemv|gbmv) (a10|s10|gen9) (tiny|large) (hw|emulator) [bitstream]"
}
...
if [ "$2" != "gemm" -a "$2" != "conv" -a "$2" != "capsule" -a "$2" != "pairhmm" -a "$2" != "gbmv" -a "$2" != "gemv" ]; then
    show_usage
    return
else
    workload="$2"
