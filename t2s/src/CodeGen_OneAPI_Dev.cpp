...
string CodeGen_OneAPI_Dev::CodeGen_OneAPI_C::print_reinterpret(Type type, const Expr &e) {
    ostringstream oss;
...
void CodeGen_OneAPI_Dev::CodeGen_OneAPI_C::visit(const Call *op) {
    if (op->is_intrinsic(Call::bool_to_mask)) {
        ...
        if (string_pipe_index[string_pipe_index.size() - 1] == ',') {
            string_pipe_index.pop_back();
        }
        id = '_' + unique_name('_');
...
void CodeGen_OneAPI_Dev::CodeGen_OneAPI_C::visit_binop(Type t, const Expr &a, const Expr &b, const char *op) {
...
// EmitOneAPIFunc
std::string CodeGen_OneAPI_Dev::EmitOneAPIFunc::print_reinterpret(Type type, const Expr &e) {
    ostringstream oss;
...
void CodeGen_OneAPI_Dev::CodeGen_OneAPI_C::DeclareChannels::visit(const Realize *op) {
    if (ends_with(op->name, ".channel") || ends_with(op->name, ".channel.array")) {
        ...
        if (pipeBounds[pipeBounds.size() - 1] == ',') {
            pipeBounds.pop_back();
        }
