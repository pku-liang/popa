...
public:
    ...
    void visit(const Provide *op) override {
        ...
        for (size_t i = 0; i < loop_vars.size(); i++) {
            ...
            if (is_const_one(loop_bounds[i].extent)) {
...
class GPUBufferBuilder
{
    ...
    void get_loop_bounds(string name, Region &init_bounds, Region &iter_bounds) {
        ...
        for (size_t i = 0; i < sz; i++) {
            if (extract_last_token(loop_vars[i]) == store_at
                && (reuse_args.empty() || !is_const_one(reuse_args[i]))) {
    ...
    -> GPUBufInfo::LOAD {
        ...
        if (rw_len != vec_ext) {
            ...
            for (size_t i = 0; i < im_args[name].size(); i++) {
                ...
                user_assert(is_const_zero(min) || equal(min, region[i].min))
                    << "The coalesced loop must be align with other loops\n";
            }
            expand_elem(name, inner, idx, last_arg);
            for (size_t i = 0; i < im_args[name].size(); i++) {
                user_assert(is_const_one(inner[i].extent) || i == last_arg.arg_idx
    ...
    -> GPUBufInfo::LOAD {
        ...
        if (region.size() <= 2) {
            ...
            internal_assert(is_const_zero(region[0].min));
            region[0].extent  = 0;
...
public:
    ...
    Stmt visit(const For *op) override {
        ...
        if (!f.empty()) {
            ...
            if (!is_const_one(output.if_cond)) {
