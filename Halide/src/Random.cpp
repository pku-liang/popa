...
Expr rng32(Expr x) {
    ...
    if (const uint64_t *i = (const uint64_t *)as_const_uint(x)) {
...
Expr random_int(const vector<Expr> &e) {
    ...
    for (size_t i = 1; i < e.size(); i++) {
        ...
        const uint64_t *ir = (const uint64_t *)as_const_uint(result);
        const uint64_t *ie = (const uint64_t *)as_const_uint(e[i]);
        if (ir && ie) {
