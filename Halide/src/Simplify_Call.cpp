...
Expr Simplify::visit(const Call *op, ExprInfo *bounds) {
    ...
    if (op->is_intrinsic(Call::strict_float)) {
        ...
        uint64_t ua = 0;
        __uint128_t tmp_ua = 0;
        auto is_const_int = const_int(a, (int64_t *)&ua);
        auto is_const_uint = const_uint(a, &tmp_ua);
        if (is_const_int || is_const_uint) {
            ua = is_const_uint ? static_cast<uint64_t>(tmp_ua) : ua;
        ...
        uint64_t ub = 0;
        __uint128_t tmp_ub = 0;
        int64_t sb = 0;
        bool b_is_const_uint = const_uint(b, &tmp_ub);
        bool b_is_const_int = const_int(b, &sb);
        if (b_is_const_uint || b_is_const_int) {
            ub = b_is_const_int ? std::abs(sb) : static_cast<uint64_t>(tmp_ub);
        ...
        if (const_int(a, &ia) &&
            ...
            return make_const(op->type, ia & ib);
                   ...
                   const_uint(b, &ub)) {
            return make_const(op->type, static_cast<uint64_t>(ua) & static_cast<uint64_t>(ub));
            ...
            return Mod::make(a, make_const(a.type(), ib + 1));
                   ...
                   b.type().is_max(static_cast<uint64_t>(ub))) {
            return a;
        } else if (const_uint(b, &ub) &&
                   is_const_power_of_two_integer(make_const(a.type(), static_cast<uint64_t>(ub) + 1), &bits)) {
            return Mod::make(a, make_const(a.type(), static_cast<uint64_t>(ub) + 1));
        ...
        if (const_int(a, &ia)) {
            ...
            return make_const(op->type, ~static_cast<uint64_t>(ua));
        ...
        if (op->type == a.type()) {
            ...
            // uint -> int
            return make_const(op->type, static_cast<int64_t>(ua));
        } else if (a.same_as(op->args[0])) {
            return op;
        } else {
        ...
        if (ta.is_int() && const_int(a, &ia) && const_int(b, &ib)) {
            ...
            const uint64_t d = ua > ub ? static_cast<uint64_t>(ua) - static_cast<uint64_t>(ub)
                                       : static_cast<uint64_t>(ub) - static_cast<uint64_t>(ua);
            return make_const(op->type, d);
