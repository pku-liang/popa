...
int main()
{
    const int BAND_I = II * I;
    Halide::Runtime::Buffer<float> A(K, K), x(K), b(K);
    Halide::Runtime::Buffer<float> bandedA(BAND_I, K);

    for (size_t i = 0; i < K; i++) {
        for (size_t k = 0; k < K; k++) {
            A(i, k) = (i-k >= BAND_I) ? 0 : random()%10 + 1;
        }
    }

    for (size_t i = 0; i < BAND_I; i++) {
        for (size_t k = 0; k < K; k++) {
             bandedA(i, k) = A(i+k, k);
        }
    }
    ...
    float number_ops = 2 * (float)K * (float)K; // Total operations (GFLOP for GEMV), independent of designs
    float number_bytes = (float)K * (float)K * 4 +
                         (float)K * 4 +
                         (float)K * 4;
