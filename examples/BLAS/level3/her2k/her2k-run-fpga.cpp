...
int main()
{
    ...
    Halide::Runtime::Buffer<complex32_t> a(TOTAL_K, TOTAL_I), b(TOTAL_J, TOTAL_K);
    for (size_t i = 0; i < TOTAL_I; i++) {
        for (size_t k = 0; k < TOTAL_K; k++) {
            a(k, i) = complex32_t(1, 1);
        }
    }
    for (size_t k = 0; k < TOTAL_K; k++) {
        for (size_t j = 0; j < TOTAL_J; j++) {
            b(j, k) = complex32_t(1, 1);
        }
    }

    Halide::Runtime::Buffer<complex32_t> c(JJJ, III, JJ, II, J, I);
    her2k(a, b, c);
#ifdef TINY
         ...
         for (int jj = 0; jj < JJ; jj++)
            for (int iii = 0; iii < III; iii++)
             for (int jjj = 0; jjj < JJJ; jjj++) {
                ...
                complex32_t golden{0.0f, 0.0f};
                for (int k = 0; k < TOTAL_K; k++) {
                    golden += a(k, total_i) * b(total_j, k).conj() + b(k, total_j) * a(total_i, k).conj();
                }
                // Since matrix C is a Hermitian matrix, we only compute its upper triangle, 
                // and use its conjugate for the lower triangle.
                if (j < i) { // lower triangle
                    //printf("golden = (%f, %f), c = (%f, %f)\n", golden.re(), golden.im(), c(iii, jjj, ii, jj, i, j).re(),
                    //       c(iii, jjj, ii, jj, i, j).im());
                    assert(fabs(golden.re() - c(iii, jjj, ii, jj, i, j).re()) <= 0.005 * fabs(golden.re()) &&
                           fabs(golden.im() + c(iii, jjj, ii, jj, i, j).im()) <= 0.005 * fabs(golden.im()));
                } else { // upper triangle
                    //printf("golden = (%f, %f), c = (%f, %f)\n", golden.re(), golden.im(), c(jjj, iii, jj, ii, j, i).re(),
                    //        c(jjj, iii, jj, ii, j, i).im());
    ...
    double number_ops = 16.0 * TOTAL_I * TOTAL_J * TOTAL_K + 2.0 * TOTAL_I * TOTAL_J;
    double number_bytes = (double)(KKK * III) * (double)(KK * II) * (double)(K * J * I) * 8 +
                          (double)(KKK * JJJ) * (double)(KK * JJ) * (double)(K * J * I) * 8 +
                          (double)(III * II * I) * (double)(JJJ * JJ * J) * 8;
