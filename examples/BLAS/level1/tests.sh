...
if [ "$target" == "a10" -o "$target" == "s10" ]; then
    ...
    ./test.sh $location asum $target tiny emulator
    ./test.sh $location axpy $target tiny emulator
    ./test.sh $location dot $target tiny emulator
    ./test.sh $location dotc $target tiny emulator
    ./test.sh $location sdsdot $target tiny emulator
    ./test.sh $location nrm2 $target tiny emulator
    ./test.sh $location rot $target tiny emulator
    ./test.sh $location scal $target tiny emulator
    ./test.sh $location iamax $target tiny emulator
else
