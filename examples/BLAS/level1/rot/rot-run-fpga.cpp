...
int main()
{
    Halide::Runtime::Buffer<float> x(TOTAL_I), y(TOTAL_I);
    Halide::Runtime::Buffer<float> outx(II, I), outy(II, I);

    for (size_t i = 0; i < TOTAL_I; i++) {
        x(i) = random();
        y(i) = random();
    }
    float s = random(), c = random();

    rot(x, y, c, s, outx, outy);

#ifdef TINY
    for (size_t i = 0; i < TOTAL_I; i++) {
        float golden_x = c*x(i) + s*y(i);
        float golden_y = c*y(i) - s*x(i);
        assert(fabs(golden_x - outx(i)) <= 0.005 * fabs(golden_x));
        assert(fabs(golden_y - outy(i)) <= 0.005 * fabs(golden_y));
    }
#else
    ...
    float number_ops = 6 * TOTAL_I; // Total operations (GFLOP for GEMV), independent of designs
