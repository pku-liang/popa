...
bool Simplifier::const_int(const Expr &e, int64_t *i) {
...
bool Simplifier::const_uint(const Expr &e, uint64_t *u) {
...
Expr Simplifier::visit(const Reinterpret* op) {
  DEBUG() << "simpifying reinterpret: " << std::endl;
...
Stmt Simplifier::visit(const For* op) {
  ...
  return For::make(op->name, mutate(op->min), mutate(op->extent), op->for_type, op->partition_policy, op->device_api, mutate(op->body));
}
...
Stmt Simplifier::visit(const HoistedStorage *op) {
  DEBUG() << "simpifying HoistedStorage" << std::endl;
  Stmt body = mutate(op->body);
