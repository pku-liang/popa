...
bool CodeGen_MLIR::MaybeRegenerateToIndexOrInteger(mlir::Value& v, bool to_integer, mlir::Type target_type) noexcept {
  ...
  if (auto temp = mlir::dyn_cast_or_null<mlir::arith::ConstantIntOp>(v_op)) {
    auto int_attr = temp.getValue().dyn_cast_or_null<mlir::IntegerAttr>();
    if (int_attr) {
      if (to_integer) {
          v = builder_.create<mlir::arith::ConstantIntOp>(v_op->getLoc(),
                                                          int_attr.getInt(),
                                                          target_type);
      } else {
          v = builder_.create<mlir::arith::ConstantIndexOp>(v_op->getLoc(),
                                                          int_attr.getInt());
      }
      return true;
    }
  ...
  if ((IsValidAffineExpr(op->a) && IsValidAffineExpr(op->b))) {
    ...
    if (op->type.lanes() == 1) {
      if (!v1.getType().isIndex() && !v2.getType().isIndex()) {
        ...
        if (v1_bits > v2_bits) {
          v2 = builder_.create<mlir::arith::ExtSIOp>(builder_.getUnknownLoc(),
                                                    v1.getType(),
                                                    v2);
        } else if (v1_bits < v2_bits) {
          v1 = builder_.create<mlir::arith::ExtSIOp>(builder_.getUnknownLoc(),
                                                    v2.getType(),
                                                    v1);
        }
      }
      ...
      if (op->type.lanes() == 1) {
        if (!MaybeRegenerateToIndexOrInteger(v1, v2, true)) {
          if (v1.getType().isIndex()) {
            v2 = builder_.create<mlir::arith::IndexCastOp>(builder_.getUnknownLoc(), builder_.getIndexType(), v2);
          } else if (v2.getType().isIndex()) {
            v1 = builder_.create<mlir::arith::IndexCastOp>(builder_.getUnknownLoc(), builder_.getIndexType(), v1);
          } else {
...
template<typename T>
void CodeGen_MLIR::GenerateCmpOp(const T* op, mlir::arith::CmpFPredicate type_f, mlir::arith::CmpIPredicate type_i) noexcept {
  Type type;
  ...
  if (type.is_int_or_uint()) {
    ...
    value_ = builder_.create<mlir::arith::CmpIOp>(builder_.getUnknownLoc(), type_i, v1, v2);
  } else {
    value_ = builder_.create<mlir::arith::CmpFOp>(builder_.getUnknownLoc(), type_f, v1, v2);
  }
}
...
void CodeGen_MLIR::compile(const Module& module) {
  ...
  // // func_pm.addPass(mlir::bufferization::createPromoteBuffersToStackPass(1024, 64, 3));
  // // func_pm.addPass(mlir::bufferization::createBufferDeallocationPass());
  ...
  module_.print(os);
  std::cerr << "The output IR file is: runme.mlir. Exiting..." << std::endl;
...
void CodeGen_MLIR::CompileFunction(const LoweredFunc& func) noexcept {
  ...
  auto mlir_func = builder_.create<mlir::func::FuncOp>(builder_.getUnknownLoc(), func.name, func_type);
...
void CodeGen_MLIR::CompileBuffer(const Buffer<>& buffer) noexcept {
  ...
  mlir::Attribute buf_attr = mlir::DenseElementsAttr::getFromRawBuffer(buf_type, llvm::ArrayRef<char>(buf_data));
  ...
  auto global_memref = builder_.create<mlir::memref::GlobalOp>(
                         ...
                         mlir::bufferization::BufferizeTypeConverter{}.convertType(buf_type).cast<mlir::MemRefType>(),
...
void CodeGen_MLIR::visit(const IntImm* op) {
  ...
  value_ = builder_.create<mlir::arith::ConstantIntOp>(builder_.getUnknownLoc(), op->value, ConvertToMlirType(op->type));
}
...
void CodeGen_MLIR::visit(const FloatImm* op) {
  ...
  llvm::APFloat temp(op->value);
  if (op->type.bits() == 32) {
    bool loses;
    temp.convert(llvm::APFloat::IEEEsingle(), llvm::APFloat::rmNearestTiesToEven, &loses);
  }
  auto type = ConvertToMlirType(op->type);
  value_ = builder_.create<mlir::arith::ConstantFloatOp>(builder_.getUnknownLoc(), temp, type.dyn_cast<mlir::FloatType>());
}

void CodeGen_MLIR::visit(const StringImm* op) {
  DEBUG() << "string" << std::endl;
  value_ = builder_.create<mlir::arith::ConstantOp>(builder_.getUnknownLoc(),
...
void CodeGen_MLIR::visit(const For* op) {
  ...
  if (is_valid_affine_op) {
...

    {
      ...
      mlir::AffineForOp::ensureTerminator(loop.getRegion(), builder_, builder_.getUnknownLoc());
    }
  } else {
    mlir::Value lower = Codegen(op->min);
    if (!lower.getType().isIndex()) {
      if (!MaybeRegenerateToIndexOrInteger(lower)) {
        lower = builder_.create<mlir::arith::IndexCastOp>(builder_.getUnknownLoc(), builder_.getIndexType(), lower);
      }
    }
    ...
    if (!upper.getType().isIndex()) {
      if (!MaybeRegenerateToIndexOrInteger(upper)) {
        upper = builder_.create<mlir::arith::IndexCastOp>(builder_.getUnknownLoc(), builder_.getIndexType(), upper);
      }
    }

    mlir::Value step = builder_.create<mlir::arith::ConstantIndexOp>(builder_.getUnknownLoc(), 1);
    if (!step.getType().isIndex()) {
      if (!MaybeRegenerateToIndexOrInteger(step)) {
        step = builder_.create<mlir::arith::IndexCastOp>(builder_.getUnknownLoc(), builder_.getIndexType(), step);
      }
    }
...

    {
      ...
      mlir::scf::ForOp::ensureTerminator(loop.getRegion(), builder_, builder_.getUnknownLoc());
    }
  }
}
...
void CodeGen_MLIR::visit(const Add* op) {
  ...
  GenerateArithOp<Add, mlir::arith::AddFOp, mlir::arith::AddIOp>(op);
...
void CodeGen_MLIR::visit(const Sub* op) {
  ...
  GenerateArithOp<Sub, mlir::arith::SubFOp, mlir::arith::SubIOp>(op);
...
void CodeGen_MLIR::visit(const Mul* op) {
  ...
  GenerateArithOp<Mul, mlir::arith::MulFOp, mlir::arith::MulIOp>(op);
...
void CodeGen_MLIR::visit(const Div* op) {
  ...
  GenerateArithOp<Div, mlir::arith::DivFOp, mlir::arith::DivSIOp>(op);
...
void CodeGen_MLIR::visit(const Mod* op) {
  ...
  GenerateArithOp<Mod, mlir::arith::RemFOp, mlir::arith::RemSIOp>(op);
...
void CodeGen_MLIR::visit(const Let* op) {
  ...
  if (!(TableContains<VariableT>(op->name) || global_buffers_.contains(op->name))) {
    ...
    if (constants_int_.count(op->name)) {
      value_ = builder_.create<mlir::arith::ConstantIntOp>(builder_.getUnknownLoc(),
                                                 constants_int_[op->name],
                                                 builder_.getIndexType());
...
void CodeGen_MLIR::visit(const LetStmt* op) {
  ...
  if (!(TableContains<VariableT>(op->name) || global_buffers_.contains(op->name))) {
    DEBUG() << "not in tableeeeeeeeee: " << op->name << std::endl;
    if (constants_int_.count(op->name)) {
      value_ = builder_.create<mlir::arith::ConstantIndexOp>(builder_.getUnknownLoc(),
                                                            constants_int_[op->name]);
      TablePush(op->name, VariableT({value_, VarType::kAffineSym}));
    } else {
      lazy_evals_.push(op->name, op->value);
...
void CodeGen_MLIR::visit(const IfThenElse* op) {
  ...
  mlir::scf::IfOp::ensureTerminator(ifThenElse.getThenRegion(), builder_, builder_.getUnknownLoc());
  ...
  if (withElse) {
    ...
    mlir::scf::IfOp::ensureTerminator(ifThenElse.getElseRegion(), builder_, builder_.getUnknownLoc());
  }
}
  ...
  if (type_src_halide.is_float()) {
    if (type_dst_halide.is_float()) {
      if (type_src_halide.bits() < type_dst_halide.bits()) {
        value_ = builder_.create<mlir::arith::ExtFOp>(builder_.getUnknownLoc(),
                                                      ConvertToMlirType(type_dst_halide), value_);
      } else {
        assert(false && "no fp truncation op available");
      }
    } else if (type_dst_halide.is_int_or_uint()) {
      if (type_dst_halide.is_uint()) {
        value_ = builder_.create<mlir::arith::FPToUIOp>(builder_.getUnknownLoc(),
                                                        ConvertToMlirType(type_dst_halide), value_);
      } else {
        value_ = builder_.create<mlir::arith::FPToSIOp>(builder_.getUnknownLoc(),
                                                        ConvertToMlirType(type_dst_halide), value_);
      }
    } else {
    ...
    if (type_dst_halide.is_float()) {
      if (value_.getType().isIndex()) {
        if (!MaybeRegenerateToIndexOrInteger(value_, true, ConvertToMlirType(type_src_halide))) {
          value_ = builder_.create<mlir::arith::IndexCastOp>(builder_.getUnknownLoc(),
                                                      ConvertToMlirType(type_src_halide), value_);
        }
      }
      if (type_src_halide.is_uint()) {
        value_ = builder_.create<mlir::arith::UIToFPOp>(builder_.getUnknownLoc(),
                                                        ConvertToMlirType(type_dst_halide), value_);
      } else {
        value_ = builder_.create<mlir::arith::SIToFPOp>(builder_.getUnknownLoc(),
                                                        ConvertToMlirType(type_dst_halide), value_);
      }
    } else if (type_dst_halide.is_int_or_uint()) {
      if (type_src_halide.bits() < type_dst_halide.bits()) {
        if (value_.getType().isIndex()) {
          ...
          if (type_src_halide.is_uint()) {
            value_ = builder_.create<mlir::arith::ExtSIOp>(builder_.getUnknownLoc(),
                                                          type_dest_mlir, value_);
          } else {
            value_ = builder_.create<mlir::arith::ExtUIOp>(builder_.getUnknownLoc(),
                                                          type_dest_mlir, value_);
          }
        }
      } else if (type_src_halide.bits() > type_dst_halide.bits()) {
        value_ = builder_.create<mlir::arith::TruncIOp>(builder_.getUnknownLoc(),
                                                        type_dest_mlir, value_);
      }
    } else {
...
void CodeGen_MLIR::visit(const EQ* op) {
  ...
  GenerateCmpOp<EQ>(op, mlir::arith::CmpFPredicate::OEQ, mlir::arith::CmpIPredicate::eq);
...
void CodeGen_MLIR::visit(const NE* op) {
  ...
  GenerateCmpOp<NE>(op, mlir::arith::CmpFPredicate::ONE, mlir::arith::CmpIPredicate::ne);
...
void CodeGen_MLIR::visit(const LT* op) {
  ...
  mlir::arith::CmpIPredicate pred = op->a.type().is_int()? mlir::arith::CmpIPredicate::slt : mlir::arith::CmpIPredicate::ult;
  GenerateCmpOp<LT>(op, mlir::arith::CmpFPredicate::OLT, pred);
...
void CodeGen_MLIR::visit(const LE* op) {
  ...
  mlir::arith::CmpIPredicate pred = op->a.type().is_int()? mlir::arith::CmpIPredicate::sle : mlir::arith::CmpIPredicate::ule;
  GenerateCmpOp<LE>(op, mlir::arith::CmpFPredicate::OLE, pred);
...
void CodeGen_MLIR::visit(const GT* op) {
  ...
  mlir::arith::CmpIPredicate pred = op->a.type().is_int()? mlir::arith::CmpIPredicate::sgt : mlir::arith::CmpIPredicate::ugt;
  GenerateCmpOp<GT>(op, mlir::arith::CmpFPredicate::OGT, pred);
...
void CodeGen_MLIR::visit(const GE* op) {
  ...
  mlir::arith::CmpIPredicate pred = op->a.type().is_int()? mlir::arith::CmpIPredicate::sge : mlir::arith::CmpIPredicate::uge;
  GenerateCmpOp<GE>(op, mlir::arith::CmpFPredicate::OGE, pred);
...
void CodeGen_MLIR::visit(const And* op) {
  ...
  value_ = builder_.create<mlir::arith::AndIOp>(builder_.getUnknownLoc(), a.getType(), a, b);
}
...
void CodeGen_MLIR::visit(const Or* op) {
  ...
  value_ = builder_.create<mlir::arith::OrIOp>(builder_.getUnknownLoc(), a.getType(), a, b);
}
...
void CodeGen_MLIR::visit(const Select* op) {
  ...
  if (!MaybeRegenerateToIndexOrInteger(true_branch, false_branch, true)) {
    if (true_branch.getType().isIndex()) {
      false_branch = builder_.create<mlir::arith::IndexCastOp>(builder_.getUnknownLoc(), builder_.getIndexType(), false_branch);
    } else if (false_branch.getType().isIndex()) {
      true_branch = builder_.create<mlir::arith::IndexCastOp>(builder_.getUnknownLoc(), builder_.getIndexType(), true_branch);
    } else {
      // assert(false && "unimplemented type comparison(currently only int and index)");
    }
  }

  value_ = builder_.create<mlir::arith::SelectOp>(builder_.getUnknownLoc(),
                                                cond,
                                                true_branch,
                                                false_branch);
...
void CodeGen_MLIR::visit(const Load* op) {
  ...
  if (!index.getType().isIndex()) {
    if (!MaybeRegenerateToIndexOrInteger(index)) {
      index = builder_.create<mlir::arith::IndexCastOp>(builder_.getUnknownLoc(),
                                                 builder_.getIndexType(), index);
    }
  }
...
void CodeGen_MLIR::visit(const Ramp* op) {
  if (is_const(op->stride) && !is_const(op->base)) {
    ...
    value_ = builder_.create<mlir::arith::ConstantOp>(builder_.getUnknownLoc(), builder_.getZeroAttr(type));
    for (int i = 0; i < op->type.lanes(); i++) {
      if (i > 0) {
        if (op->type.is_float()) {
          base = builder_.create<mlir::arith::AddFOp>(builder_.getUnknownLoc(), base.getType(), base, stride);
        } else {
          base = builder_.create<mlir::arith::AddIOp>(builder_.getUnknownLoc(), base.getType(), base, stride);
        }
      }
...
void CodeGen_MLIR::visit(const Call* op) {
  ...
  if (op->is_intrinsic(Call::abs)) {
    ...
    if (a.getType().isIndex()) {
      if (!MaybeRegenerateToIndexOrInteger(a, true, t)) {
        a = builder_.create<mlir::arith::IndexCastOp>(builder_.getUnknownLoc(), t, a);
      }
    }
    if (b.getType().isIndex()) {
      if (!MaybeRegenerateToIndexOrInteger(b, true, t)) {
        b = builder_.create<mlir::arith::IndexCastOp>(builder_.getUnknownLoc(), t, b);
      }
    }

    value_ = builder_.create<mlir::arith::OrIOp>(builder_.getUnknownLoc(),
                                                a.getType(), a, b);
    ...
    value_ = builder_.create<mlir::arith::XOrIOp>(builder_.getUnknownLoc(),
                                                a.getType(), a, b);
    ...
    value_ = builder_.create<mlir::arith::AndIOp>(builder_.getUnknownLoc(),
                                                  a.getType(), a, b);
    ...
    value_ = builder_.create<mlir::arith::XOrIOp>(builder_.getUnknownLoc(), a.getType(), a, b);
    ...
    if (op->args[1].type().is_uint()) {
      ...
      value_ = builder_.create<mlir::arith::ShLIOp>(builder_.getUnknownLoc(),
    ...
    if (op->args[1].type().is_uint()) {
      ...
      if (op->type.is_int()) {
        value_ = builder_.create<mlir::arith::ShRSIOp>(builder_.getUnknownLoc(), t, a, b);
      } else {
        value_ = builder_.create<mlir::arith::ShRUIOp>(builder_.getUnknownLoc(), t, a, b);
      }
    } else {
  ...
  //     Codegen(e);
    ...
    if (op->type.is_int()) {
      value_ = builder_.create<mlir::arith::DivSIOp>(builder_.getUnknownLoc(), a.getType(), a, b);
    } else if (op->type.is_uint()) {
      value_ = builder_.create<mlir::arith::DivUIOp>(builder_.getUnknownLoc(), a.getType(), a, b);
    } else {
    ...
    if (op->type.is_int()) {
      value_ = builder_.create<mlir::arith::RemSIOp>(builder_.getUnknownLoc(), a.getType(), a, b);
    } else if (op->type.is_uint()) {
      value_ = builder_.create<mlir::arith::RemUIOp>(builder_.getUnknownLoc(), a.getType(), a, b);
    } else {
      assert(false && "can only be integer");
    }
  ...
  //   value_ = builder_.create<mlir::arith::FloorFOp>(builder_.getUnknownLoc(),
  ...
  //                                ModulusRemainder());
    ...
    value_ = builder_.create<mlir::arith::ConstantOp>(builder_.getUnknownLoc(), builder_.getBoolAttr(true));
    return;
  } else {
...
void CodeGen_MLIR::visit(const Store* op) {
  ...
  if (val_to_store.getType().isIndex()) {
    val_to_store = builder_.create<mlir::arith::IndexCastOp>(builder_.getUnknownLoc(), ConvertToMlirType(op->value.type()), val_to_store);
  }

  if (!index.getType().isIndex()) {
    if (!MaybeRegenerateToIndexOrInteger(index)) {
      index = builder_.create<mlir::arith::IndexCastOp>(builder_.getUnknownLoc(), builder_.getIndexType(), index);
    }
  }
