...
void MainFuncGenerator::DeclareUtilities() noexcept {
  ...
  func = builder_.create<mlir::func::FuncOp>(builder_.getUnknownLoc(), "print_flops", func_type, visability);
  module_.push_back(func);

  func_type = builder_.getFunctionType({builder_.getF64Type()}, {});
  func = builder_.create<mlir::func::FuncOp>(builder_.getUnknownLoc(), "print_time_elapsed", func_type, visability);
  module_.push_back(func);

  func_type = builder_.getFunctionType({builder_.getIndexType()}, {});
  func = builder_.create<mlir::func::FuncOp>(builder_.getUnknownLoc(), "printIndex", func_type, visability);
  module_.push_back(func);

  func_type = builder_.getFunctionType({builder_.getI64Type()}, {});
  func = builder_.create<mlir::func::FuncOp>(builder_.getUnknownLoc(), "printI64", func_type, visability);
  module_.push_back(func);

  func_type = builder_.getFunctionType({builder_.getI32Type()}, {});
  func = builder_.create<mlir::func::FuncOp>(builder_.getUnknownLoc(), "printI32", func_type, visability);
  module_.push_back(func);
  func = builder_.create<mlir::func::FuncOp>(builder_.getUnknownLoc(), "printIU32", func_type, visability);
  module_.push_back(func);

  func_type = builder_.getFunctionType({builder_.getIntegerType(16)}, {});
  func = builder_.create<mlir::func::FuncOp>(builder_.getUnknownLoc(), "printI16", func_type, visability);
  module_.push_back(func);
  func = builder_.create<mlir::func::FuncOp>(builder_.getUnknownLoc(), "printIU16", func_type, visability);
  module_.push_back(func);

  func_type = builder_.getFunctionType({builder_.getIntegerType(8)}, {});
  func = builder_.create<mlir::func::FuncOp>(builder_.getUnknownLoc(), "printI8", func_type, visability);
  module_.push_back(func);
  func = builder_.create<mlir::func::FuncOp>(builder_.getUnknownLoc(), "printIU8", func_type, visability);
  module_.push_back(func);

  func_type = builder_.getFunctionType({builder_.getF64Type()}, {});
  func = builder_.create<mlir::func::FuncOp>(builder_.getUnknownLoc(), "printF64", func_type, visability);
  module_.push_back(func);

  func_type = builder_.getFunctionType({builder_.getF32Type()}, {});
  func = builder_.create<mlir::func::FuncOp>(builder_.getUnknownLoc(), "printF32", func_type, visability);
  module_.push_back(func);

  func_type = builder_.getFunctionType({}, {builder_.getF64Type()});
  func = builder_.create<mlir::func::FuncOp>(builder_.getUnknownLoc(), "rtclock", func_type, visability);
  module_.push_back(func);

  func_type = builder_.getFunctionType({}, {});
  func = builder_.create<mlir::func::FuncOp>(builder_.getUnknownLoc(), "printOpen", func_type, visability);
  module_.push_back(func);

  func_type = builder_.getFunctionType({}, {});
  func = builder_.create<mlir::func::FuncOp>(builder_.getUnknownLoc(), "printClose", func_type, visability);
  module_.push_back(func);

  func_type = builder_.getFunctionType({}, {});
  func = builder_.create<mlir::func::FuncOp>(builder_.getUnknownLoc(), "printSpace", func_type, visability);
  module_.push_back(func);

  func_type = builder_.getFunctionType({}, {});
  func = builder_.create<mlir::func::FuncOp>(builder_.getUnknownLoc(), "printComma", func_type, visability);
  module_.push_back(func);

  func_type = builder_.getFunctionType({}, {});
  func = builder_.create<mlir::func::FuncOp>(builder_.getUnknownLoc(), "printNewline", func_type, visability);
  module_.push_back(func);
}
...
void MainFuncGenerator::GenerateBody() noexcept {
  ...
  auto mlir_func = builder_.create<mlir::func::FuncOp>(builder_.getUnknownLoc(), "main", func_type);
  ...
  for (int i = 0; i < input_count; ++i) {
    ...
    switch (arg_type) {
      case ArgType::kInt:
        ...
        input = builder_.create<mlir::arith::ConstantIntOp>(builder_.getUnknownLoc(), ((IntArg*)arg.base_val)->value, builder_.getIntegerType(((IntArg*)arg.base_val)->arg_bits));
        break;
      case ArgType::kUInt:
        // attr = builder_.getIntegerAttr(builder_.getIntegerType(((UIntArg*)arg.base_val)->arg_bits), ((UIntArg*)arg.base_val)->value);
        input = builder_.create<mlir::arith::ConstantIntOp>(builder_.getUnknownLoc(), ((UIntArg*)arg.base_val)->value, builder_.getIntegerType(((UIntArg*)arg.base_val)->arg_bits));
        break;
      case ArgType::kFloat: {
        llvm::APFloat temp(((FloatArg*)arg.base_val)->value);
        if (((FloatArg*)arg.base_val)->arg_bits == 32) {
          ...
          input = builder_.create<mlir::arith::ConstantFloatOp>(builder_.getUnknownLoc(), temp, builder_.getF32Type());
        } else if (((FloatArg*)arg.base_val)->arg_bits == 64) {
          // attr = builder_.getFloatAttr(builder_.getF64Type(), ((FloatArg*)arg.base_val)->value);
          input = builder_.create<mlir::arith::ConstantFloatOp>(builder_.getUnknownLoc(), temp, builder_.getF64Type());
        } else {
          assert(false);
        }
        // input = builder_.create<mlir::arith::ConstantIntOp>(builder_.getUnknownLoc(), attr);
        break;
      }
      case ArgType::kBuffer:
  ...
  for (int i = input_count; i < arguments_.size(); ++i) {
    ...
    if (auto ty = output_type.getElementType().dyn_cast_or_null<mlir::FloatType>()) {
      llvm::APFloat temp(ty.getFloatSemantics(), "0");
      value_to_fill = builder_.create<mlir::arith::ConstantFloatOp>(builder_.getUnknownLoc(), temp, ty);
    } else if (auto ty = output_type.getElementType().dyn_cast_or_null<mlir::IntegerType>()) {
      value_to_fill = builder_.create<mlir::arith::ConstantIntOp>(builder_.getUnknownLoc(), 0, ty);
    }
    builder_.create<mlir::linalg::FillOp>(builder_.getUnknownLoc(), value_to_fill, output);
  ...
  args1 = {};
  start = builder_.create<mlir::func::CallOp>(builder_.getUnknownLoc(), llvm::StringRef("rtclock"), ret_type, args1).getResult(0);

  ret_type = {};
  builder_.create<mlir::func::CallOp>(builder_.getUnknownLoc(), cg_mlir_.func_names_[0], ret_type, args);

  ret_type = {builder_.getF64Type()};
  args1 = {};
  end = builder_.create<mlir::func::CallOp>(builder_.getUnknownLoc(), llvm::StringRef("rtclock"), ret_type, args1).getResult(0);

  elapsed = builder_.create<mlir::arith::SubFOp>(builder_.getUnknownLoc(), builder_.getF64Type(), end, start);

  ret_type = {};
  args1 = {elapsed};
  builder_.create<mlir::func::CallOp>(builder_.getUnknownLoc(), llvm::StringRef("print_time_elapsed"), ret_type, args1);
...
void MainFuncGenerator::GeneratePrintMemref(mlir::Value memref, ArgType type) noexcept {
  ...
  std::function<void ()> gen_loop_nest = [&]() {
    if (index >= shape.size()) {
      ...
      args = {val};
      if (elem_type.isF32()) {
        builder_.create<mlir::func::CallOp>(builder_.getUnknownLoc(), llvm::StringRef("printF32"), ret_type, args);
      } else if (elem_type.isF64()) {
        builder_.create<mlir::func::CallOp>(builder_.getUnknownLoc(), llvm::StringRef("printF64"), ret_type, args);
      } else if (elem_type.isInteger(32)) {
        if (type == ArgType::kUInt) {
          builder_.create<mlir::func::CallOp>(builder_.getUnknownLoc(), llvm::StringRef("printIU32"), ret_type, args);
        } else {
          builder_.create<mlir::func::CallOp>(builder_.getUnknownLoc(), llvm::StringRef("printI32"), ret_type, args);
        }
      } else if (elem_type.isInteger(64)) {
        if (type == ArgType::kUInt) {
          builder_.create<mlir::func::CallOp>(builder_.getUnknownLoc(), llvm::StringRef("printIU64"), ret_type, args);
        } else {
          builder_.create<mlir::func::CallOp>(builder_.getUnknownLoc(), llvm::StringRef("printI64"), ret_type, args);
        }
      } else if (elem_type.isInteger(16)) {
        if (type == ArgType::kUInt) {
          builder_.create<mlir::func::CallOp>(builder_.getUnknownLoc(), llvm::StringRef("printIU16"), ret_type, args);
        } else {
          builder_.create<mlir::func::CallOp>(builder_.getUnknownLoc(), llvm::StringRef("printI16"), ret_type, args);
        }
      } else if (elem_type.isInteger(8)) {
        if (type == ArgType::kUInt) {
          builder_.create<mlir::func::CallOp>(builder_.getUnknownLoc(), llvm::StringRef("printIU8"), ret_type, args);
        } else {
          builder_.create<mlir::func::CallOp>(builder_.getUnknownLoc(), llvm::StringRef("printI8"), ret_type, args);
        }
      }
      ret_type = {};
      args = {};
      builder_.create<mlir::func::CallOp>(builder_.getUnknownLoc(), llvm::StringRef("printComma"), ret_type, args);
    ...
    if (shape[index] != -1) {
      upper = builder_.create<mlir::arith::ConstantIndexOp>(builder_.getUnknownLoc(), shape[index]);
    } else {
      upper = builder_.create<mlir::memref::DimOp>(builder_.getUnknownLoc(), memref, index);
    }
    lower = builder_.create<mlir::arith::ConstantIndexOp>(builder_.getUnknownLoc(), 0);
    step = builder_.create<mlir::arith::ConstantIndexOp>(builder_.getUnknownLoc(), 1);
    ...
    ret_type = {};
    args = {};
    builder_.create<mlir::func::CallOp>(builder_.getUnknownLoc(), llvm::StringRef("printNewline"), ret_type, args);
  };
  gen_loop_nest();
  builder_.create<mlir::func::CallOp>(builder_.getUnknownLoc(), llvm::StringRef("printNewline"), ret_type, args);
}
